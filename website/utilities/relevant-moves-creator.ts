...
export function createRelevantMoves(
  move: Move,
  character: Character
): RelevantMove[] {
  const relatedMoves = relevantMoves.find((groupedMoves) =>
    groupedMoves.includes(move.normalizedName)
  );
  if (!relatedMoves) {
    return [];
  }
  ...
  const moves = character.moves.filter(
    (storedMove) =>
      relatedMoves.findIndex(
        (relatedMove) => storedMove.normalizedName === relatedMove
      ) !== -1 && storedMove.normalizedName !== move.normalizedName
  );
